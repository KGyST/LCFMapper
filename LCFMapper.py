...
class GUIAppSingleton(XMLProcessorBase):
  ...
  async def _start(self):
    ...
    try:
      ...
      await self.run_converter("x2l", tempXMLDir, tempGDLDir, self.SourceImageDirName.get())

      await self.run_converter("createcontainer", tempGDLDir, self.TargetLCFPath.get(), self.SourceImageDirName.get())
  ...
  async def run_converter(self, command: str, source_dir: str, target_dir: str, img_dir: str = ''):
